...
var helpers = function () {
    var isDir = function isDir(filename) {
        var stats = fs.statSync(filename);
        return stats && stats.isDirectory();
    };

    var registerPartials = function registerPartials(dir, base) {
        base = base || dir;
    ...
    var registerPartial = function registerPartial(filename, base) {
    ...
    var isHandleBars = function isHandleBars(filename) {
        return ['hbs'].indexOf(filename.split('.').pop()) !== -1;
    };

    var partialName = function partialName(filename, base) {
...
module.exports = function (sourceFolder, targetFolder, configuration, filePathsArg) {
    ...
    var compileHandleBars = function compileHandleBars(file, cb) {
    ...
    var rename = function rename(file, cb) {
    ...
    var logChange = function logChange(file, cb) {
        cb(null, file);
    };

    var action = function action(cb) {
        vfs.src(filePathsArg).pipe(map(compileHandleBars)).pipe(map(rename)).pipe(vfs.dest(targetFolder)).on('finish', function () {
            cb();
        });
    };
